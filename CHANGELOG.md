...
### Added
- Yolo agent mode (`/mode yolo`): skip all permission prompts, including dangerous `bash`, with a red input area as a reminder. `Shift+Tab` cycles through `build` → `plan` → `yolo`. `Guard` denials (system paths, `.gitignore`, home dotfiles) still block. Yolo has the full build tool registry (no plan-style tool stripping) and exits cleanly via `/mode build`.
...
### Added
- Add Z.ai GLM-5.3 and OpenCode Go LongCat model support with provider-specific thinking parameters.
...
### Fixed
- Bump Go to 1.25.13 to resolve stdlib vulnerabilities.

### Added
- Add a demo GIF and move the demo recording guide to the docs.
